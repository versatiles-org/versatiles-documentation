...
- **[Editorial Tools](https://versatiles.org/node-versatiles-svelte/map-editor)**: A web-based suite for newsrooms — create locator maps, area maps and point maps for stories on local events, exclusion zones, and more.
...
- **[Map Animation](https://github.com/versatiles-org/versatiles-map-animation)** (prototype): A browser-based editor for composing keyframe camera animations and annotations on a VersaTiles map. Animations are shared as a URL hash, embeddable via `<iframe>`, and can be rendered to MP4 with a Docker image.
...
- **VersaTiles-Studio**: A desktop application in Rust for converting geographic data into vector tiles with size optimisation.
