...
const filtered = computed(() =>
  data.showcases.filter(s => {
    ...
    if (selectedTags.value.size > 0 && !s.tags.some(t => selectedTags.value.has(t))) return false;
    return true;
  })
);
...
<template>
  <div class="showcases">
    <div class="filters" v-if="data.countries.length > 1">
      ...
      <div class="filter-group">
        ...
        <button
          ...
          @click="toggleTag(tag)"
        >{{ formatTag(tag) }}</button>
      </div>
      <button v-if="hasFilters" class="clear-btn" @click="clearFilters">Clear filters</button>
    </div>

    <p class="result-count">{{ filtered.length }} showcase{{ filtered.length !== 1 ? 's' : '' }}</p>
    ...
    <div class="card-grid">
      <a
        ...
        class="card"
      >
        <div class="card-thumb" v-if="item.thumbnail">
        ...
        <div class="card-body">
          ...
          <div class="card-tags">
            <span v-for="tag in item.tags" :key="tag" class="card-tag">{{ formatTag(tag) }}</span>
          </div>
